...
import { LogOut, Shield, Folder, Plus, Loader2 } from 'lucide-react';
...
export const Dashboard = () => {
  ...
  // Función para obtener las carpetas reales desde el backend
  const obtenerCarpetas = async () => {
    try {
      const res = await fetch('http://localhost:4000/api/v1/passwords');
      if (res.ok) {
        const datos = await res.json();
        // Extraemos categorías únicas de las contraseñas guardadas
        const cats = datos.map((c: any) => c.category).filter((cat: string) => cat);
        setCarpetas(Array.from(new Set([...cats])));
      }
    } catch (e) {
  ...
  const crearCarpetaLocal = (e: React.FormEvent) => {
    e.preventDefault();
    if (nombreCarpeta.trim() && !carpetas.includes(nombreCarpeta)) {
      // La añadimos a la vista. Se volverá permanente al guardar la primera cuenta dentro.
      setCarpetas([...carpetas, nombreCarpeta.trim()]);
      setNombreCarpeta('');
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {carpetas.map(folder => (
              ...
              >
                ...
                <span className="font-black text-slate-700 uppercase text-xs tracking-[0.2em]">{folder}</span>
              </div>
            ))}
          </div>
        )}
      ...
      {mostrarModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-6">
          <form onSubmit={crearCarpetaLocal} className="bg-white p-10 rounded-[40px] shadow-2xl w-full max-w-sm">
