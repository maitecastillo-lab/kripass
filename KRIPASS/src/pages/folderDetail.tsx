...
import { Plus, Eye, EyeOff, Trash2, Shield, Loader2, ArrowLeft, Pencil } from 'lucide-react';
...
export const FolderDetail = () => {
  ...
  const [cuentaEditar, setCuentaEditar] = useState<PasswordAccount | null>(null);
  ...
  // EDITAR (NUEVO)
  const abrirEditar = (item: PasswordAccount) => {
    setCuentaEditar({ ...item, password: item.password || '' });
    setMostrarEditar(true);
  };

  const actualizarCuenta = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!cuentaEditar || !cuentaEditar.id) return;

    try {
      // Solo enviamos los campos que queremos actualizar
      const datosActualizar: Partial<PasswordAccount> = {
        site: cuentaEditar.site,
        username: cuentaEditar.username,
        category: cuentaEditar.category,
      };

      // Solo enviamos la contraseña si realmente cambió algo
      if (cuentaEditar.password && cuentaEditar.password.trim() !== '') {
        datosActualizar.password = cuentaEditar.password;
      }

      await passwordClient.update(cuentaEditar.id, datosActualizar);
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
            ...
            {items.filter(i => i.site.toLowerCase().includes(busqueda.toLowerCase())).map(item => (
              <div key={item.id} className="bg-white p-8 rounded-[35px] border border-slate-100 shadow-sm relative hover:shadow-md transition-all">
                ...
                <div className="absolute top-6 right-6 flex gap-2">
                  <button onClick={() => abrirEditar(item)} className="text-slate-300 hover:text-blue-500 transition-colors">
                    <Pencil size={18} />
                  </button>
                  <button onClick={() => eliminarCuenta(item.id!)} className="text-slate-200 hover:text-red-500 transition-colors">
                    <Trash2 size={20} />
                  </button>
                </div>
      ...
      {/* MODAL EDITAR */}
      {mostrarEditar && cuentaEditar && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <form onSubmit={actualizarCuenta} className="bg-white p-10 rounded-[40px] w-full max-w-md shadow-2xl border border-slate-100">
            <h3 className="text-2xl font-black mb-6 text-slate-800 flex items-center gap-2">
              <Pencil size={22} className="text-blue-500" /> EDITAR CREDENCIAL
            </h3>
            <div className="space-y-4">
              <input
                required
                ...
                value={cuentaEditar.site}
                onChange={e => setCuentaEditar({ ...cuentaEditar, site: e.target.value })}
              />
              <input
                required
                className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 focus:ring-2 focus:ring-blue-500 outline-none"
                placeholder="Usuario / Email"
                value={cuentaEditar.username}
                onChange={e => setCuentaEditar({ ...cuentaEditar, username: e.target.value })}
              />
              <input
                className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 focus:ring-2 focus:ring-blue-500 outline-none"
                type="text"
                placeholder="Nueva contraseña (o deja la actual)"
                value={cuentaEditar.password || ''}
                onChange={e => setCuentaEditar({ ...cuentaEditar, password: e.target.value })}
              />
              <p className="text-[10px] text-slate-400 italic">💡 Puedes ver y editar la contraseña si quieres cambiarla.</p>
            </div>
            <button className="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold mt-8 hover:bg-blue-700 transition-all shadow-lg uppercase tracking-tight">
              Guardar cambios
            </button>
            <button type="button" onClick={() => { setMostrarEditar(false); setCuentaEditar(null); }} className="w-full mt-4 text-slate-400 font-bold text-sm">
              CANCELAR
            </button>
          </form>
        </div>
